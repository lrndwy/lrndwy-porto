...
export default function TetrisLoading({
  ...
  const rotateShape = useCallback((shape: number[][]): number[][] => {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < cols; j++) rotated[j][rows - 1 - i] = shape[i][j]
    }
    return rotated
  }, [])
  ...
  const clearFullLines = useCallback(() => {
    setGrid(prevGrid => {
      ...
      prevGrid.forEach((row, index) => { if (row.every(cell => cell.filled)) linesToClear.push(index) })
      if (linesToClear.length > 0) {
        setIsClearing(true)
        const newGrid = prevGrid.map((row, rowIndex) => (
          linesToClear.includes(rowIndex)
            ? row.map(cell => ({ ...cell, color: 'bg-black dark:bg-white animate-pulse opacity-50' }))
            : row
        ))
        setTimeout(() => {
          setGrid(currentGrid => {
