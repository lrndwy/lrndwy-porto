...
const defaultSteps: readonly Step[] = [
  { id: "1", name: "Item 1", title: "Feature 1", description: "Feature 1 description" },
  { id: "2", name: "Item 2", title: "Feature 2", description: "Feature 2 description" },
  { id: "3", name: "Item 3", title: "Feature 3", description: "Feature 3 description" },
] as const
...
function useNumberCycler(totalSteps: number = TOTAL_STEPS, interval: number = 3000) {
...
function useIsMobile() {
  ...
  useEffect(() => {
    ...
    const isMobileUserAgent = Boolean(/Android|BlackBerry|iPhone|iPad|iPod|Opera Mini|IEMobile|WPDesktop/i.exec(userAgent))
    const isDev = process.env.NODE_ENV !== "production"
    if (isDev) setIsMobile(isSmall || isMobileUserAgent)
    else setIsMobile(isSmall && isMobileUserAgent)
  }, [])
  return isMobile
}
...
const stepVariants: Variants = { inactive: { scale: 0.8, opacity: 0.5 }, active: { scale: 1, opacity: 1 } }
...
const AnimatedStepImage = ({ preset = "fadeInScale", delay = 0, onAnimationComplete, ...props }: AnimatedStepImageProps) => {
...
function FeatureCard({ bgClass, children, step }: CardProps & { children: React.ReactNode; step: number }) {
  ...
  return (
    <motion.div className="animated-cards relative w-full rounded-[16px]" onMouseMove={handleMouseMove} style={{
      ...
      <div className={clsx(
        ...
        <div className="m-6 md:m-10 min-h-[360px] md:min-h-[420px] w-full">
          <AnimatePresence mode="wait">
            <motion.div
              key={step}
              className="flex w-full items-center justify-center flex-col gap-3"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.3, ease: [0.23, 1, 0.32, 1] }}
            >
              {/* Heading umum dihilangkan sesuai permintaan */}
            </motion.div>
          </AnimatePresence>
...
function Steps({ steps: stepData, current, onChange }: { steps: readonly Step[]; current: number; onChange: (index: number) => void }) {
  return (
            ...
            <motion.li key={`${step.name}-${stepIdx}`} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className={cn("relative z-50 rounded-full px-3 py-1 transition-all duration-300 ease-in-out md:flex", isCompleted ? "bg-neutral-500/20" : "bg-neutral-500/10")}>
              <div className={cn("group flex w-full cursor-pointer items-center focus:outline-none focus-visible:ring-2", (isFuture || isCurrent) && "pointer-events-none")} onClick={() => onChange(stepIdx)}>
                <span className="flex items-center gap-2 text-sm font-medium">
                  <motion.span initial={false} animate={{ scale: isCurrent ? 1.2 : 1 }} className={cn("flex h-4 w-4 shrink-0 items-center justify-center rounded-full duration-300", isCompleted && "bg-brand-400 text-white dark:bg-brand-400", isCurrent && "bg-brand-300/80 text-neutral-400 dark:bg-neutral-500/50", isFuture && "bg-brand-300/10 dark:bg-neutral-500/20")}>
                    {isCompleted ? (
                  ...
                  <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className={clsx("text-xs sm:text-sm font-medium duration-300", isCompleted && "text-muted-foreground", isCurrent && "text-lime-300 dark:text-lime-500", isFuture && "text-neutral-500")}>
                    {step.title}
                  </motion.span>
                </span>
              </div>
...
export const Component = ({ image, stepsData, step1img1Class = defaultClasses.step1img1, step1img2Class = defaultClasses.step1img2, step2img1Class = defaultClasses.step2img1, step2img2Class = defaultClasses.step2img2, step3imgClass = defaultClasses.step3img, ...props }: ComponentProps) => {
  const { currentNumber: step, increment } = useNumberCycler()
  const [isAnimating, setIsAnimating] = useState(false)
  const steps = stepsData ?? defaultSteps as Step[]
            ...
            <motion.div className="relative w-full h-full" onAnimationComplete={handleAnimationComplete}>
              <div className="absolute inset-0 grid place-items-center text-center p-6">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-neutral-300">{steps[0]?.description}</p>
                </div>
              </div>
            </motion.div>
          )
            ...
            <motion.div className="relative w-full h-full" onAnimationComplete={handleAnimationComplete}>
              <div className="absolute inset-0 grid place-items-center text-center p-6">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-neutral-300">{steps[1]?.description}</p>
                </div>
              </div>
            </motion.div>
          )
        case 2:
          return <div className="absolute inset-0 grid place-items-center text-center p-6"><div className="space-y-2"><h3 className="text-2xl font-semibold">{steps[2]?.title}</h3><p className="text-sm text-neutral-300">{steps[2]?.description}</p></div></div>
        case 3:
          return (
            <motion.div className={clsx("absolute inset-0 grid place-items-center text-center p-6 text-2xl font-bold")}{...ANIMATION_PRESETS.fadeInScale} onAnimationComplete={handleAnimationComplete}>
              <div>
                <h3 className="text-2xl font-semibold">{steps[3]?.title}</h3>
                <p className="text-sm text-neutral-300 mt-2">{steps[3]?.description}</p>
              </div>
            </motion.div>
          )
        default:
